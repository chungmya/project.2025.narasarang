 ...
 * [U21/00/00][html] body.linkShow : N/A 등 모든 링크 클릭 가능
 ...
 * --------------------------------------
 * @Todo
* [Todo][25/04/07] 소팅이후 새로 고침에도 계속 소팅 되게 남아 있도록
 * [Todo][25/04/07] 소팅 이후 현재 보여지는 화면만 colorbox 보여지도록
 ...
 * --------------------------------------
 * @Log
 * [v1.5.2]	[25/01/13] modernizr, detectizr 제거
 * [v1.5.1]	[24/05/03] colorbox resize __g 선언으로 변경
 * [v1.5.1]	[24/05/02] idSplit 자리수 직접 입력으로 변경
 * [v1.5.1]	[24/05/02] 각 카테고리별 통계 오류 수정: calculator_cate()
 * [v1.5]		[23/07/20] 자르는 글자수 및 앞글자 제외 수 수정 및 추가
...
var Worklist = (function(){
	...
	const setMap = (options) => {
		...
		, __manmonth = {
			...
			, holiday :[
				{date: '2025/1/27', label: '임시공휴일'}
				, {date: '2025/1/28', label: '설연휴'}
				, {date: '2025/1/29', label: '설날'}
				, {date: '2025/1/30', label: '설연휴'}
				, {date: '2025/3/3', label: '대체휴일'}
				, {date: '2025/5/1', label: '근로자의날'}
				, {date: '2025/5/5', label: '부처님/어린이날'}
				, {date: '2025/5/6', label: '대체휴일'}
				, {date: '2025/6/6', label: '현충일'}
				, {date: '2025/8/15', label: '광복절'}
				, {date: '2025/10/3', label: '개천절'}
				, {date: '2025/10/6', label: '추석'}
				, {date: '2025/10/7', label: '추석연휴'}
				, {date: '2025/10/8', label: '대체휴일'}
				, {date: '2025/10/9', label: '한글날'}
				, {date: '2025/12/25', label: '성탄철'}
				, {date: '2026/1/1', label: '신ㄴ'}
			]
			, corder:[
				{
					id:'corder01'
				...
				, {
					...
					, endDate: '2025/12/31'
					, vacation : [
						{date: '2025/7/9', label: '연차:건강검진', day:1}
					]
				}
				, {
					id:'corder04'
					...
					, endDate: '2025/12/31'
					, vacation : [
						// {date: '2025/4/2', label: '대체휴가', day:1}
					]
				}
			]
		}
		...
		, __g = {
			...
			, focusObj: null
			, navNum: document.querySelectorAll('.tab_nav > ul > li').length - 1 // 1Depth length
			, winScroll: 0 //현재 스크롤
			, filterObj: {
				delete: true
				, group: true
			...
			, colorbox:{
				ARresize : [280, 320, 360, 375, 560, 768, 1000, 1200, 1280, 1440, 1600, 1920]
			}
			, logObj: {
	...
	var setHtml = () => {
		htmlMap = {
										...
										<tr>
											...
											<td class="filterOption">
												...
												<p class="ml10"><strong> Group</strong>
													<label><input type="radio" name="ch_group" id="group01" value="true" checked /> Yes</label>
													<label><input type="radio" name="ch_group" id="group02" value="false" /> No</label>
												</p>
												<p class="ml10"><strong> Child</strong>
													<label><input type="radio" name="ch_child" id="child01" value="true" checked /> Show</label>
													<label><input type="radio" name="ch_child" id="child02" value="false" /> Hide</label>
												</p>
											</td>
										</tr>
										<tr>
			...
			dimm: document.body.insertAdjacentHTML('beforeend', '<div class="dimmed"></div>')
		};
	};
	...
	var project = (function(){
		...
		// const setTitle = () => {
			...
			const projectName = $h1.innerHTML || Util.getProjectName();
			let listName = document.querySelector('body').dataset.page || 'worklist';
			listName = listName.charAt(0).toUpperCase() + listName.slice(1);

			// title
			$title.innerHTML = '';
			$title.textContent = `${listName} | ${projectName}`;

			// h1
			$h1.innerHTML = `<a href="worklist.html">${projectName}<small> ${listName}</small></a>`;

			// tab_title
			// h3 요소에 header > .tab_nav 기본 가져옮
			// 텍스트 있을경우 Depth1 (텍스트)
			// rMap.tab_navList.forEach((tab, i) => {
			document.querySelectorAll('.tab_nav  ul > li').forEach((tab, i) => {
				const tabTitle = tab.querySelector('a > span')?.textContent || '';
				const $targetTab = rMap.tab_content[i];
				const $h3 = $targetTab?.querySelector('h3');
				let h3Title = $h3?.textContent || '';

				if (h3Title) {
					$h3.dataset.dir = h3Title;
					h3Title = `(${$h3.textContent.trim()})`;
				}

				if ($h3) {
					$h3.textContent = '';
					$h3.insertAdjacentHTML('afterbegin', `<span class="title" style="color:#666;">${tabTitle} </span>${h3Title}`);
				}
			});
		}
	...
	var directory = (function(){
		...
		function isIMG(str){
			var fileType = ['jpg', 'jpeg', 'png', 'gif', 'ico', 'svg'];
			for(var i = 0; i <= fileType.length-1; i++){
				if(str.trim().toLowerCase() == fileType[i]) return true;
			}
			return false;
	...
	var View = (function(){
		var viewAll = function(_target){
			__g.settingOption.pageScroll.is_viewAll = true;
		}
		, viewOne = function(_target){
			__g.settingOption.pageScroll.is_viewAll = false;
		}
		, watch = function(){
			if(__g.settingOption.pageScroll.is_viewAll){
				viewAll();
			}else{
				viewOne();
			}
	...
	var Nav = (function(){
			...
			rMap.tab_navList.each(function(i){
				...
				$depth1.find('> a').on({
					...
					, click : function(e){
						...
						//Hash Mode
						if(!!!__g.settingOption.pageScroll.scrollMode){
							//전체보기
		...
		, initModule = function(){
			...
			if(Setting.bool(__g.settingObj.option.scroll)) __g.settingOption.pageScroll.scrollMode = true;
			else __g.settingOption.pageScroll.scrollMode = false;

			setTimeout(function(){
				initDepth1();//top position setting
			},500);

			if(__g.settingOption.pageScroll.scrollMode){//scrollMode
				location.hash = '';
	...
	var ColorBox = (function(){
		...
		var setMap = function(){
			...
			for(var i =0; i < __g.colorbox.ARresize.length; i++){
				var _zindex = 1000 - (i * 10);
				var _width = __g.colorbox.ARresize[i];;
				var _marginLeft = (_width / 2) * -1;
				// var _style = '"z-index:'+_zindex+';width:'+_width+'px; margin-left:'+_marginLeft+'px;"';

				var addHtml = '<span class="bar s'+_width+'" data-width="'+_width+'" style="z-index:'+_zindex+';width:'+_width+'px; margin-left:'+_marginLeft+'px;"></span>'
				if(i == 0) addHtml = '<span class="bar s'+_width+'" data-width="'+_width+'" style="z-index:'+_zindex+';width:'+_width+'px; margin-left:'+_marginLeft+'px;"><span class="info_txt"></span></span>'

				fMap.control.append(addHtml);
			}

			//재정의
			fMap.control_btn = fMap.control.find('.bar');
		...
		, colorBoxHandler = function(){
			...
			// iFrame 안 컨텐트 모바일, PC, mobile  토글 : 768 이하일경우 scrollbar 사이즈 보이게
	...
	var Filter = (function(){
		...
		function calculator(){
			...
			$contentTR.each(function(index){
				...
				//팝업 텍스트 필터링
				var popTxtFlag = (
											    $(this).find('td.path').text().slice(-1) === 'P' ||
											    $(this).find('td.path').text().slice(-1) === 'p'
												) ? true : false
				;
			...
			$resultBtn_ex.find('span').text('미완 : ' + unResultCnt);
		...
		function calculator_cate(){
			...
			$('.info_calc').each(function(){
				...
				$table.find('tr').each(function(index){
					...
					//Filter Option 구분에 따라(Delete, group)
					//----------------------------
					// Group: child group 보여지게 Numbering
					//  - true 	: (1_1, 1_2 같이 보여짐) child 삭제
					//  - false : (1, 2, 3, 같이 보여징) child 포함 

					// 천체 Count
					//----------------------------
					//01. Delete: 포함, Group: Yes
					if(__g.filterObj.delete && __g.filterObj.group) {
						if($(this).hasClass('del') && $(this).hasClass('child')){ __g.totalCnt--;
						}else if($(this).hasClass('del')){ // __g.totalCnt--;
						}else if($(this).hasClass('child')){ __g.totalCnt--;
						}
					}

					//02. Delete: 포함, Group: No (Default)
					else if(__g.filterObj.delete && !__g.filterObj.group){
						if($(this).hasClass('del') && $(this).hasClass('child')){ // __g.totalCnt++;
						}else if($(this).hasClass('del')){ // __g.totalCnt++;
						}else if($(this).hasClass('child')){ // __g.totalCnt++;
						}
					...
					if(!rdateTxtFlag){}
						...
						//01. Delete: 포함, Group: Yes
						if(__g.filterObj.delete && __g.filterObj.group) {
							if($(this).hasClass('del') && $(this).hasClass('child')){ resultCnt--;
							}else if($(this).hasClass('del')){ // resultCnt--;
							}else if($(this).hasClass('child')){ resultCnt--;
							}
						}

						//02. Delete: 포함, Group: No
						else if(__g.filterObj.delete && !__g.filterObj.group){
							if($(this).hasClass('del') && $(this).hasClass('child')){ // resultCnt--;
							}else if($(this).hasClass('del')){ // resultCnt--;
							}else if($(this).hasClass('child')){ // resultCnt--;
						...
						//03. Delete: 제외, Group: Yes (Default)
						else if(!__g.filterObj.delete && __g.filterObj.group){
							if($(this).hasClass('del') && $(this).hasClass('child')){ resultCnt--;
							}else if($(this).hasClass('del')){ resultCnt--;
							}else if($(this).hasClass('child')){ resultCnt--;
	...
	var Etc = (function(){
		...
		var idSplit = function(_obj){
			var $path = rMap.tab_content.find('tbody .path > a');
			var ARsplitNum = _obj.splitPos ? _obj.splitPos : __g.settingOption.idSplit.pos;//[2,3,2,2...]

			$path.each(function(k){
				var strORI = $(this).text().replace(' ', '')
					, strSplit = ''
					, strNew = ''
					, curPos = 0
				;

				if(!strORI || $(this).closest('tr').hasClass('del')) return;

				for(var i=0; i<ARsplitNum.length; i++){
					var endPos = ARsplitNum[i] + curPos;

					strSplit = strORI.substring(curPos, endPos);
					curPos = endPos;

					if(strSplit) strNew +='<span>'+ strSplit +'</span>';
				}

				//split 문자 이외 마지막 문자열
				var strElse = strORI.substring(curPos, strORI.length)
				if(strElse) strNew += '<span>'+ strORI.substring(curPos, strORI.length) +'</span>'

				$(this).empty().html('<span class="id_split">'+strNew+'</span>');
			});
		}
	 ...
	 * --------------------------------------
	 */
	const initModule = function(options, callback){
		options = options || {
			version : 'worklist'
			, desktop:{
				setting:{
